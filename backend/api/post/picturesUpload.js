...
app.post('/uploadQr', async (request, response) => {
    let qr64Encoded = request.body.image;
    const data = qr64Encoded.replace(/^data:image\/\w+;base64,/, "");
    ...
    let responseUploadData = await s3.upload(params).promise();
    console.log(responseUploadData);
});
